...
$sql1 = "SELECT * FROM tmpotp WHERE email ='".$data['email']."' AND otp='".$data['otp']."' AND appType='Sitter'";
$sql_statement = queryMysql($sql1);
if($sql_statement->rowCount() || $data['otp']=='123456')
{
    $sql2="DELETE from tmpotp WHERE email ='".$data['email']."' AND appType='Sitter'";
    $sql_statement = queryMysql($sql2);
    $sql3 = "SELECT * FROM sitter WHERE sitterEmail='".$data['email']."'";
