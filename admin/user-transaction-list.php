...
session_start();
  ...
  include("include/global.php");
     $selectBooking =  mysqli_query($connect,"SELECT * FROM transaction WHERE transactionsTypeID='User' ORDER BY transactionsId  desc ");
   
    $n=0;
    while($rowRole = mysqli_fetch_array($selectBooking))
    {

      $sqlC = mysqli_query($connect,"SELECT * FROM user WHERE userID = '".$rowRole['transactionsUserID']."'");
      ...
      $n++; 
       $List .='<tr>
                  <td>'.$n.'</td>
                    <td>'.$rowC['userFirstname'].' '.$rowC['userLastname'].'</td>
                   <td>'.$rowC['userMobileno'].'</td>
